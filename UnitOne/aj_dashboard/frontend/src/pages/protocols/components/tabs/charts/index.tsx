...
const Charts: React.FC<any> = ({
  ...
  const {  reCounter } = useCounter();
  useEffect(() => {
    if (protocol_id) {
      http<ListType<ProtocolType>>(
        addParamsToEndpoint(getEndpoint("similar_protocols"), {
          id: protocol_id,
        })
      ).then((response) => {
  ...
  useEffect(() => {
    setProtocols(
      protocols.map((el: any) =>
        el.id == protocol_id
          ? {
              ...el,
              ["custom_sensory_panels"]: sensory,
    ...
    if (protocols.length) {
      for (let protocol of protocols) {
        ...
        for (const sensoryPanel of protocolSensoryPanel) {
          let index = dataSensoryPanel.findIndex(
            (dataSensoryPanel: any) =>
              dataSensoryPanel["taste"] == sensoryPanel.variable
          );
          if (index > -1) {
        ...
        Object.keys(protocolTasteIntensity).map((key, index) => {
          ...
          let indexm = dataTasteIntensity.findIndex(
            (dataTasteIntensity: any) => dataTasteIntensity["taste"] == key
          );
          if (indexm > -1) {
        ...
        Object.keys(protocolAromaIntensity).map((key, index) => {
          ...
          let indexm = dataAromaIntensity.findIndex(
            (dataAromaIntensity: any) => dataAromaIntensity["taste"] == key
          );
          if (indexm > -1) {
        ...
        Object.keys(protocolNutritionInfo).map((key, index) => {
          ...
          let indexm = dataNutritionInfo.findIndex(
            (dataNutritionInfo: any) => dataNutritionInfo["taste"] == key
          );
          if (indexm > -1) {
        ...
        Object.keys(protocolTextureMetrics).map((key, index) => {
          ...
          let indexm = dataTextureMetrics.findIndex(
            (dataTextureMetrics: any) => dataTextureMetrics["taste"] == key
          );
          if (indexm > -1) {
  ...
  return (
    // Every column have space of 2 unit, and in row 3 units
    <Grid container columnSpacing={2} rowSpacing={3}>
      {sensoryPanelChart.length > 0 && (
        <SensoryChart
          key={"sensory-chart-" + (state as any).counter}
          title="Sensory Panel"
          keys={protocolsKeys}
          data={sensoryPanelChart}
        />
      )}
      {tasteIntensityChart.length > 0 && (
        <SensoryChart
          key={"taste-intensity-chart-" + (state as any).counter}
          title="Taste"
          keys={protocolsKeys}
          data={tasteIntensityChart}
        />
      )}
      {aromaIntensityChart.length > 0 && (
        <SensoryChart
          key={"aroma-intensity-chart" + (state as any).counter}
          title="Aroma Intensity"
          keys={protocolsKeys}
          data={aromaIntensityChart}
        />
      )}
      {nutritionInfoChart.length > 0 && (
        <SensoryChart
          title="Nutrition Info"
          keys={protocolsKeys}
          data={nutritionInfoChart}
        />
      )}
      {textureMetricsChart.length > 0 && (
        <SensoryChart
          title="TextureMetrics"
          keys={protocolsKeys}
          data={textureMetricsChart}
        />
      )}
    </Grid>
  );
