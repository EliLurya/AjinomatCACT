...
const Design: React.FC<any> = ({
  ...
  useEffect(() => {
    http<ListType<ProjectType>>(getEndpoint("all_sensors"), {
      params: { page_size: 12 },
    }).then((response) => {
  ...
  return (
    <List>
      <ListSensory
        ...
        items={sensorsList}
        sensory={sensory}
        handleSensory={handleSensory}
      />
      <Divider />
      {Object.keys(tasteIntensity).length != 0 && (
        <ExtraList
          name="Taste intensityl"
          newTitle=""
          orgTitle="Org"
          items={tasteIntensity}
          setitems={setTasteIntensity}
        />
      )}
      <Divider />
      {Object.keys(aromaIntensity).length != 0 && (
        <ExtraList
          name="Aroma Intensity"
          newTitle=""
          orgTitle="Org"
          items={aromaIntensity}
          setitems={setAromaIntensity}
        />
      )}
      <Divider />
      {Object.keys(nutritionInfo).length != 0 && (
        <ExtraList
          ...
          orgTitle="Org"
          items={nutritionInfo}
          setitems={setNutritionInfo}
        />
      )}
      <Divider />
      {Object.keys(textureMetrics).length != 0 && (
        <ExtraList
          name="Texture Metrics"
          newTitle=""
          orgTitle="Org"
          items={textureMetrics}
          setitems={setTextureMetrics}
        />
      )}
      <Divider />
