...
const CreateEditProtocol: React.FC = () => {
  ...
  //If the edges is undefined, when create a first node ,enter in setEdges empty array
  useEffect(() => {
    if (edges === undefined) setEdges([]);
  }, [nodes]);
  console.log("(state as any).counter", edges as any);
  console.log("(state as any).nodes: ", nodes as any);
  return (
    <>
      <Stack flexDirection="row">
        <Box width="100%" style={{ height: "80ch" }} key={"nodes-" + counter}>
          <Stack
            ...
            className="list-master-actions"
            width="100%"
          >
            <Button variant="text" color="info" onClick={onDuplicate}>
              Duplicate
            </Button>
            <Button
              variant="text"
            ...
            <label htmlFor="upload-photo">
              ...
              <Button
                color="info"
                variant="text"
                className="upload_protocol_button"
                component="span"
              >
                Upload
              </Button>
